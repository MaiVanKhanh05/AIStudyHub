...
import { Sparkles, Tag, ChevronRight, BookOpen } from "lucide-react";

export default function FlashcardSetCard({ setId }) {
    ...
    useEffect(() => {
        const fetchMeta = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5000/api/flashcards/${setId}/meta`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                setMeta(res.data);
            } catch (err) {
                console.error("Failed to load flashcard metadata for card:", err);
                setError(true);
            } finally {
                setLoading(false);
            }
    ...
    if (loading) {
        return (
            <div className="my-3 p-4 bg-slate-900/60 border border-slate-800 rounded-2xl animate-pulse max-w-sm w-full select-none text-left">
                <div className="w-24 h-3 bg-purple-500/20 rounded-full mb-3" />
                <div className="w-56 h-4.5 bg-slate-850 rounded-full mb-2" />
                <div className="w-28 h-3 bg-slate-850 rounded-full mb-4" />
        ...
        return (
            <div className="my-3 p-4 bg-slate-900/40 border border-rose-900/20 rounded-2xl max-w-sm text-left select-none">
                ...
                <p className="text-[11px] text-slate-400 font-semibold leading-relaxed">
                    Không thể tải bộ thẻ ghi nhớ này hoặc bộ thẻ đã bị xóa.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="my-3 p-4 bg-white dark:bg-[#131522] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm hover:shadow-md hover:border-purple-500/30 dark:hover:border-purple-500/30 transition-all duration-300 max-w-sm text-left select-none relative group overflow-hidden">
            ...
            {/* Header Badge */}
            <div className="flex items-center gap-1.5 mb-2.5 text-purple-650 dark:text-purple-400 font-black text-[9px] uppercase tracking-widest leading-none">
                <span className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse" />
                <span>AI Flashcard Created</span>
            </div>
            ...
            {/* Count of cards */}
            <p className="text-[10px] text-slate-500 dark:text-slate-450 font-bold mb-3 uppercase tracking-wider flex items-center gap-1">
                <BookOpen className="w-3.5 h-3.5 text-purple-500/80" />
                {meta.cardCount} thẻ ôn tập tri thức
            </p>
            ...
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    navigate(`/flashcards/${meta.setId}`, {
                        state: {
