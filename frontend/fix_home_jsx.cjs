...
for(let i=0; i<lines.length; i++) {
  if(lines[i].includes('${boundary') && lines[i+1] && lines[i+1].trim().startsWith(') : (') && lines[i+2] && lines[i+2].includes('Layout: Document')) {
    brokenLineIdx = i;
    break;
  }
}
...
// Replace: keep the current line (${boundary), then insert all the fix lines, then remove the next 2 lines
const newLines = [
  ...lines.slice(0, brokenLineIdx + 1), // keep up to and including ${boundary line
  ...insertLines,
  ...lines.slice(brokenLineIdx + 3), // skip the broken ') : (' and '/* Layout...' lines
];
